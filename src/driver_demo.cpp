 ...
 * The ROS version is contributed by XintongGuo, Dalian University of Technology.
 * This version is only used for study. The driver stability cannot be guaranteed.
 *****************************************************************************/
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
DEFINE_bool(auto_gain, true, "turn on auto gain");
...
DEFINE_string(sensor_type, "XP3", "XP or XP2 or XP3 or FACE or XPIRL or XPIRL2");
...
XPDRIVER::shared_queue<XPDRIVER::ImuData> imu_data_publish_queue;
XPDRIVER::shared_queue<XPDRIVER::ImuData> imu_data_queue;
XPDRIVER::shared_queue<PublishImage> imgs_for_publish_queue;
...
void imu_data_callback(const XPDRIVER::ImuData& imu_data) {
  //cout << "imu_data_callback" << endl;
  if (run_flag) {
    imu_data_queue.push_back(imu_data);
    imu_data_publish_queue.push_back(imu_data);
  }
}
...
void thread_proc_img() {
  ...
  while (run_flag) {
    ...
    //publish queue prepare
    PublishImage img_for_publish;
    img_for_publish.l = stereo_img.l.clone();
    img_for_publish.r = stereo_img.r.clone();
    img_for_publish.ts_100us = stereo_img.ts_100us;
    imgs_for_publish_queue.push_back(img_for_publish);
...
int main(int argc, char** argv) {
  ...
  image_transport::Publisher pub_left = it.advertise("boteye_XP3/image_left", 250);
  image_transport::Publisher pub_right = it.advertise("boteye_XP3/image_right", 250);
  ros::Publisher IMU_pub = nh.advertise<sensor_msgs::Imu>("boteye_XP3/IMU_data", 1000);  

  ros::Rate loop_rate(100);//ros loop frequency
  ros::Time current_time_img, current_time_imu;
  PublishImage img_is_publishing;
  XPDRIVER::ImuData imu_data_publishing;
...
#endif
//FLAGS_record_path+="/home/guo/data";
  ...
  if (!FLAGS_headless) {
    while (nh.ok()) {
      ...
      //the img show window should not be open, because it cost a lot of time
      //if the imshow must open, do it in a new thread
/*
#ifdef __ARM_NEON__
      ...
      //needs to judge the queue is empty or not, if queue is empty 
      //imgs_for_publish_queue.wait_and_pop_to_back(&img_is_publishing) will wait and block
      //which leads the frequency of imu is limited to 25hz and the data is wrong
      if(!imgs_for_publish_queue.empty()) {
        if (imgs_for_publish_queue.wait_and_pop_to_back(&img_is_publishing))  {
          current_time_img = ros::Time::now();//using current time for ROS TimeStamp
          img_seq_ID ++;
          std_msgs::Header img_header;
          img_header.seq = img_seq_ID;
          img_header.frame_id = "image";
          img_header.stamp = current_time_img;
          //cout << img_is_publishing.ts_100us << endl;
          sensor_msgs::ImagePtr msg_left = cv_bridge::CvImage(img_header, "rgb8", img_is_publishing.l).toImageMsg();
          sensor_msgs::ImagePtr msg_right = cv_bridge::CvImage(img_header, "rgb8", img_is_publishing.r).toImageMsg();
          pub_left.publish(msg_left);
          pub_right.publish(msg_right);
        }
      }
      if(!imu_data_publish_queue.empty()) {
        if (imu_data_publish_queue.wait_and_pop_to_back(&imu_data_publishing))  {
          current_time_imu = ros::Time::now();//ROS TimeStamp
          imu_seq_ID ++;
          sensor_msgs::Imu imu_data_publish;
          // accel is in m/s^2
          // angv is in rad/s
          imu_data_publish.header.stamp = current_time_imu;
          imu_data_publish.header.frame_id = "map";
          imu_data_publish.header.seq = imu_seq_ID;
          imu_data_publish.linear_acceleration.x = imu_data_publishing.accel[0];
          imu_data_publish.linear_acceleration.y = imu_data_publishing.accel[1];
          imu_data_publish.linear_acceleration.z = imu_data_publishing.accel[2];

          imu_data_publish.angular_velocity.x = imu_data_publishing.ang_v[0];
          imu_data_publish.angular_velocity.y = imu_data_publishing.ang_v[1];
          imu_data_publish.angular_velocity.z = imu_data_publishing.ang_v[2];
          IMU_pub.publish(imu_data_publish);
        }
      }
    ros::spinOnce();
    loop_rate.sleep();
